...
class Auth_AD {
    ...
    public function login($username, $password) {
        ...
        if ($continue) {
            ...
            log_message(
                    'debug', 'Auth_AD: Successful login for ' . $entries['cn'][0] . '(' . $username . ') from IP ' . $this->ci->input->ip_address()
            );
    ...
    $username, $firstName = '', $surname = '', $jobTitle = '', $phone = '', $agentID = '', // For future readers: this is a Sykes-specific custom AD attribute
            $department = '', $ou = null
    ) {
        // preset the return marker
        $return = false;
        ...
        if ($continue) {
            ...
            $return = ldap_add(
                    $this->_ldap_conn, 'CN=' . $firstName . ' ' . $surname . ',' . $ou, array(
